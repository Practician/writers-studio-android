...
// Продолжение того же адаптированного чек-листа (см. NARRATIVE_ARCHITECTURE_CHECKLIST
// выше): уровень связности между абзацами (discourse flow) — ещё одна вещь, которую
// поверхностный regex-аудит не ловит. Тоже адаптация идей источника своими словами.
export const DISCOURSE_FLOW_CHECKLIST = `Уровень связности между абзацами — тоже выдаёт машинный текст, отдельно от штампов и архитектуры сюжета:
- Не веди абзацы одной линейной цепочкой «что случилось → почему → что вышло → что это значит» — добавь хотя бы одно сравнение (тот же эпизод или человек в другой раз) или возражение (кто-то не согласен с тем, что было сказано абзацем раньше).
- Меняй «текстуру» соседних сцен: плотная сцена — потом короткая и быстрая; насыщенный диалог — потом сжатое изложение. Не держи одну интонацию на всём протяжении.
- Реплика не обязана всегда закрывать абзац; список не обязан быть строго из трёх пунктов; переход между сценами не должен каждый раз использовать одну и ту же связку.
- Подбирай имена из мира именно этой истории, а не общие «безопасные» — пусть фамилии, прозвища и то, как разные персонажи зовут одного и того же человека, несут социальный смысл.`;

// Group D рубрики StoryScope + «человеческие» маркеры — то, что при сжатии в первую
// версию чек-листа выпало, хотя это и есть позитивные, а не только запретительные ориентиры.
export const HUMAN_POSITIVE_MARKERS_CHECKLIST = `Позитивные ориентиры человеческого письма (не всё сразу, выбери уместное):
- Упомяни что-то по-настоящему конкретное и существующее (книгу, автора, песню, деталь мира), а не только обобщённый образ.
- Введи важного персонажа впервые через его реплику, без предварительного описания внешности.
- Придержи главное открытие ближе к концу, а не раскрывай его сразу после середины.
- Не делай итоговую оценку героя однозначно доброй или осуждающей — амбивалентность ближе к человеческому письму, чем чёткий вердикт.
- Изредка допустимо лёгкое «подмигивание» читателю или самой форме рассказа — но не системно, максимум один раз.`;

// Специфичные тики конкретных моделей — из измерений на реальных релизах (в т.ч.
// DeepSeek V3.2 и Gemini 3), не общие рассуждения. Применимо только когда пишущая
// модель совпадает: тики другой модели можно только испортить чужой текст.
export function modelFingerprintGuidance(provider?: string, model?: string): string {
  const m = String(model || "").toLowerCase();
  if (provider === "nvidia" && m.includes("deepseek")) {
    return `\n\nОсобенности именно этой модели (DeepSeek), которые стоит сознательно нарушить:
- Не выкладывай весь нужный контекст заранее — придержи бэкстори и роняй её посреди сцены, а не равномерными порциями.
- Не своди все сильные эмоции только к поведению — иногда просто назови чувство словом или дай короткую вложенную мысль героя.
