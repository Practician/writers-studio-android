...
export const GEMINI_LITERARY_MODELS: readonly LiteraryModelProfile[] = [
  ...
  { id: "gemini-2.5-flash", label: "Gemini 2.5 Flash — запасная квота (авто)", description: "У модели отдельная дневная квота бесплатного тарифа. Приложение само переключается на неё, когда свежие модели упираются в лимит или недоступны, и само возвращается обратно; вручную выбирать профиль не нужно. Ключам новых проектов Google отвечает HTTP 404 — тогда модель запоминается как недоступная для этого ключа и больше не тратит попытку." },
  { id: "gemini-2.5-flash-lite", label: "Gemini 2.5 Flash-Lite — отдельный бакет (авто)", description: "Облегчённая модель со СВОИМ дневным лимитом (≈1000 запросов в сутки, 15 в минуту), не пересекающимся с квотой Flash: когда лимит исчерпан у 3.8 и 2.5 Flash, эта модель всё ещё отвечает. Темп ниже, поэтому идёт после основных профилей." },
  { id: "gemini-3.1-flash-lite", label: "Gemini 3.1 Flash-Lite — новее и дешевле (авто)", description: "Свежая Lite-модель с отдельной квотой бесплатного тарифа (≈1000 запросов в сутки, 15 в минуту). Подключается автоматически, когда основные Flash-профили упёрлись в лимит; для правок и продолжений сцен качества хватает." },
  { id: "gemini-2.0-flash", label: "Gemini 2.0 Flash — старый надёжный бакет (авто)", description: "Проверенная модель с самым ёмким отдельным бесплатным бакетом (≈1500 запросов в сутки, 15 в минуту, 1M токенов в минуту). Последняя в цепочке ротации: резерв, когда все свежие профили на паузе. Ключам новых проектов может отвечать 404 — тогда пропускается для этого ключа." },
];
