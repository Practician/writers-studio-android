...
describe("labyrinthCanon", () => {
  it("seed has bible marker, plan, rings rule, ch5–8 content", () => {
    ...
    const ch1 = story.chapters.find((c) => c.id === "lab-ch-1");
    const ch5 = story.chapters.find((c) => c.id === "lab-ch-5");
    const ch6 = story.chapters.find((c) => c.id === "lab-ch-6");
    const ch7 = story.chapters.find((c) => c.id === "lab-ch-7");
    const ch8 = story.chapters.find((c) => c.id === "lab-ch-8");
    const ch11 = story.chapters.find((c) => c.id === "lab-ch-11");
    assert.ok(ch1?.content && ch1.content.length > 5_000, "глава 1 перенесена из FB2");
    ...
    assert.ok(ch5?.content && ch5.content.length > 10_000, "глава 5 перенесена из FB2");
    assert.ok(ch6?.content && ch6.content.length > 20_000, "глава 6 перенесена из FB2");
    assert.ok(ch7?.content && ch7.content.length > 20_000, "глава 7 перенесена из FB2");
    assert.ok(ch8?.content && ch8.content.length > 20_000, "глава 8 перенесена из FB2");
    assert.ok(ch11?.content && ch11.content.length > 10_000, "глава 11 перенесена из FB2");
  });
  ...
  it("merge patches existing labyrinth ch5–6 stubs and bible", () => {
    ...
    assert.ok(ch6.summary.includes("источник заряда"));
    assert.ok(ch7.content.length > 10_000, "короткая устаревшая заглушка обновлена текстом из FB2");
  ...
  it("merge keeps substantive manual edits of chapter 6", () => {
    ...
    assert.ok(ch6.content.length < 5_000, "содержательная ручная правка не заменяется импортом");
  });
  ...
  it("merge restores deleted chapters and orders all 20 slots", () => {
    ...
    const ch6 = s.chapters.find((c) => /глава\s*6/i.test(c.title))!;
    assert.ok(ch6.content.length > 20_000, "гл.6 из FB2 заменила короткий старый текст");
  ...
  it("ensureLabyrinthChapterSlots restores ch7–8 on renamed book", () => {
    ...
    assert.ok(fixed.chapters.find((c) => /глава\s*6/i.test(c.title))!.content.length > 20_000);
    const ch8 = fixed.chapters.find((c) => /глава\s*8/i.test(c.title))!;
    assert.ok(ch8.content.length > 500, "пустой слот 8 заполняется seed");
    assert.ok(ch8.content.length > 10_000, "глава 8 перенесена из FB2");
  });
  ...
  it("merge fills empty chapter 8 from seed without wiping user ch7", () => {
    ...
    assert.ok(ch8.content.length > 10_000);
  });
});
